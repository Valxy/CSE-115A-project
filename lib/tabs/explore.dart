...
class _ExploreTabState extends State<ExploreTab> {
  ...
  static Future<String?> getTitle(int i, String category) async {
    ...
    List<Movie> movieList = await wrapper.getPopularMovies();
  ...
  static Future<Movie> getPoster(int i, String category) async {
    TmdbApiWrapper wrapper = TmdbApiWrapper();
    List<Movie> movieList = await wrapper.getPopularMovies();
    Movie movieItem = await movieList[i];
      ...
      child: new Column(
        children: <Widget> [
          Container(
            width: 200,
            height: 200,
            margin: const EdgeInsets.only(left: 0.0,top: 10.0, bottom: 10.0, right:0.0),
          ...
          Container(
            margin: const EdgeInsets.only(left: 0.0,top: 5.0, bottom: 5.0, right:0.0),
            child: FutureBuilder<String?>(
              future: getTitle(index, category), // a previously-obtained Future<String> or null
              builder: (BuildContext context, AsyncSnapshot<String?> snapshot) {
                return Text('${snapshot.data}', 
                  style: const TextStyle( color: Color.fromARGB(255, 0, 0, 0), fontWeight: FontWeight.bold, fontSize: 20.0)
                );
              },
            ),
          ),
            ...
            child: FutureBuilder<int?>(
              ...
              builder: (BuildContext context, AsyncSnapshot<int?> snapshot) {
                return Text('Runtime: ${snapshot.data}',
                  style: const TextStyle( color: Color.fromARGB(255, 0, 0, 0), fontWeight: FontWeight.bold, fontSize: 16.0 )
                );
              },
            ),
          ),
          Container(
            margin: const EdgeInsets.only(left: 0.0,top: 5.0, bottom: 5.0, right:0.0),
            child: FutureBuilder<String?>(
              future: getReleaseDate(index, category), // a previously-obtained Future<String> or null
              builder: (BuildContext context, AsyncSnapshot<String?> snapshot) {
